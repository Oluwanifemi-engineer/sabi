 ...
 */
export default function PhotoIntake({ onExtracted }: PhotoIntakeProps) {
  ...
  function handleFile(file: File) {
    ...
    reader.onload = async () => {
      ...
      try {
        ...
        if (!res.ok || data.error) throw new Error(data.error ?? "Could not read the letter.");
    ...
    return (
      <div className="rounded-xl border-2 overflow-hidden fade-in" style={{ borderColor: "var(--teal)" }}>
        <div className="relative">
          ...
          <img src={preview} alt="Captured letter" className="w-full max-h-64 object-contain bg-gray-50" />
          {loading && (
            <div className="absolute inset-0 bg-white/80 flex flex-col items-center justify-center gap-2">
              ...
              <p className="text-sm font-semibold" style={{ color: "var(--teal-deep)" }}>Reading your letter…</p>
            </div>
          )}
        </div>
        {!loading && (
  ...
  return (
    <div className="space-y-3">
      ...
      >
        ...
        <p className="font-semibold mb-1">Take a photo of the letter</p>
        <p className="text-sm opacity-60">Opens your camera on mobile, or choose a file</p>
      </div>
      {error && (
